        ...
        <div className="flex flex-col mx-auto justify-center items-center w-full h-screen">
          <div className="mt-auto mx-auto text-center py-10 px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col items-center justify-center gap-y-10">
              <p className="mt-3 max-w-lg  text-gray-600 dark:text-gray-400 text-center">{getLocalMessage("welcome")}</p>
              <Form {...(form as any)}>
                <form className="space-y-4 max-w-md" onSubmit={form.handleSubmit(handleSaveChanges)}>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-col gap-y-2">
                        ...
                        <FormControl>
                          <LanguageSelector
                            placeholder={getLocalMessage("select_language")}
                  ...
                  <Button disabled={isLoading} className="mt-10  w-full" type="submit">
                    {isLoading ? <RotateCw className="mr-2 h-4 w-4 animate-spin" /> : getLocalMessage("save")}
                  </Button>
                </form>
              </Form>
            </div>
